...
conn.reply(m.chat, '✿︎ Este Comando está deshabilitado por mi creador.', m, )
return
}
...
browser: ['Janeth Sub-Bot', 'Edge', '2.0.0'],
auth: {
creds: state.creds,
...
await parentw.reply(conn.user.jid, "✿︎ Conexión perdida con el servidor.", m)
}
}
...
await parentw.reply(m.chat, args[0] ? '✿︎ Conectado con éxito al WhatsApp.' : '✿︎ Vinculaste un Sub-Bot con éxito.', m, )
await sleep(5000)
if (args[0]) {
return
}
await parentw.reply(conn.user.jid, `✿︎ *Para volver a vincular un sub Bot use su token*`, m, )
}
}
