const handler = async (m, { conn, text }) => {
    ...
    if (numberMatches) {
        ...
        return conn.sendMessage(m.chat, {text: `✿︎ *Formato de usuario no reconocido. Responda a un mensaje, etiquete a un usuario o escriba su número de usuario.*`}, {quoted: fkontak});
    }        
        ...
        if (!global.global.db.data.users[user] || global.global.db.data.users[user] == '') {
            return conn.sendMessage(m.chat, {text: `✿︎ *El usuario @${userNumber} no se encuentra en mi base de datos.*`, mentions: [user]}, {quoted: fkontak});
         }
        delete global.global.db.data.users[user];
        conn.sendMessage(m.chat, {text: `✿︎ *Éxito Todos Los Datos Del User: @${userNumber} Ya Fuerón Eliminados De Mi Base De Datos.*`, mentions: [user]}, {quoted: fkontak});
};
handler.tags = ['owner'];
