...
class FakeShutdownEvent(object):
    """Class to  a threading.Event.isSet so that home of this module
...
def shell():
    ...
    if not args.mini:
        printer('✿︎ *Iniciando prueba...*', quiet)
        printer('✿︎ *Buscando servidor...*', quiet)
        try:
            speedtest.get_servers(servers=args.server, exclude=args.exclude)
        except NoMatchedServers:
            raise SpeedtestCLIError(
                '☁️ *No hay servidores coincidentes:* %s' %
                ', '.join('%s' % s for s in args.server)
            )
        except (ServersRetrievalError,) + HTTP_ERRORS:
            printer('☁️ *No se pudo obtener la lista de servidores.*', error=True)
            raise SpeedtestCLIError(get_exception())
    ...
    printer('☆ *Servidor:* %(sponsor)s\n🔮 *Ubicación:* %(name)s [%(d)0.2f km] '
            '\nꕥ️ *Latencia:* %(latency)s ms' % results.server, quiet)
    ...
    if args.download:
        ...
        printer('✿︎ *Omitiendo la prueba de descarga.*', quiet)
